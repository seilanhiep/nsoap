...
Surgical procedures done per year: `r sum(tsurgery$byhfl$staff)`

Major surgical procedures done per year: `r fmajor.surgeries`

Minor Surgical procedures done per year `r fminor.surgeries`
...
All Surgical facilities offering bellwether procedures: `r sum(allbell$percent, na.rm = TRUE)`
...
Bellwether procedures done per year: `r sum(ds.bellwether.per$byhfl$staff)`
...
Total peri operation death: `r ntperi.operation`

Total operating theatre procedures performed: `r ntoperating.theatre`

Surgery mortality rate per 100,000 population: `r nsurg.mortality.rate`

Surgery mortality rate: `r p.nsurg.mortality.rate`
...
```{r}
gt.denwf.hfl = den.wforce$byhfl %>% 
  select(c(-4)) %>% 
  pivot_wider(names_from = geninfo_hcfacilitylevel, values_from = staff) %>%
  mutate(Total = rowSums(across(where(is.numeric)), na.rm = TRUE)) %>% 
  select(workforces, `National Hospital`, `Provincial Hospital`, `District Hospital`,Private, Total)
wf.table(gt.denwf.hfl,"**Density of Workforce**","*by hospital facility level*")
```

#### graph

```{r}
group_ggplot(den.wforce$byhfl, 
...
```{r}
gt.denwf.hft = den.wforce$byhft %>% 
  select(c(-4)) %>% 
  pivot_wider(names_from = geninfo_hcfacilitytype, values_from = staff) %>% 
  mutate(Total = rowSums(across(where(is.numeric)), na.rm = TRUE)) %>% 
  select(workforces, CPA3,CPA2,`Private Hospital`,Policlinic,Clinic, Total)
wf.table(gt.denwf.hft,"**Density of Workforce**","*by hospital facility type*")
```

#### graph

```{r, fig.width= 10}
den.wforce$byhft$workforces <- str_wrap(den.wforce$byhft$workforces, width = 35)
  
group_ggplot(den.wforce$byhft, 
...
```{r}
gt.denwf.pro = den.wforce$byprovince %>% 
  select(c(-4)) %>% 
  pivot_wider(names_from = geninfo_province, values_from = staff) %>% 
  mutate(Total = rowSums(across(where(is.numeric)), na.rm = TRUE))

wf.table(gt.denwf.pro,"**Workforce density**","*Data from analysis*")
```

#### surgeons/Obstetricians gynaecologists/Anaesthesiologists

```{r, fig.width= 9}
surgeon = den.wforce$byprovince %>% 
  filter(workforces %in% unique(wforce$workforces)[c(1:5)])
surgeon$workforces <- str_wrap(surgeon$workforces, width = 20)
group_ggplot(surgeon, 
             geninfo_province,staff,workforces, 
             "surgeons/Obstetricians gynaecologists/Anaesthesiologists", 
             "",
             "",
             'none')
...
```{r, fig.width= 9}
doctor = den.wforce$byprovince %>% 
  filter(workforces %in% unique(wforce$workforces)[c(6:11,18)])
...
```{r, fig.width= 9}
alliance = den.wforce$byprovince %>% 
  filter(workforces %in% unique(wforce$workforces)[c(12:17)])
alliance$workforces <- str_wrap(alliance$workforces, width = 20)
group_ggplot(alliance, 
             geninfo_province,staff,workforces, "Allied health providers", "","",'none')
```
...
```{r}
group_ggplot(tallworkforce$byhfl ,
             type,staff,geninfo_hcfacilitylevel, 
...
```{r}
group_ggplot(tallworkforce$byhft ,
             type,staff,geninfo_hcfacilitytype, 
             "Specialist surgical workforce density", 
             "By hospital facility type",
             "",
...
```{r}
tallworkforce$byprovince$type = str_wrap(tallworkforce$byprovince$type, width = 20)
group_ggplot(tallworkforce$byprovince,
             geninfo_province,staff,type, 
...
Bellwether procedures per 100,000 population: `r bellwether.percapita`
...
Surgical procedures done per year `r sum(tsurgery$byhfl$staff)` Surgical procedures per 100,000 population `r sum(camsurpercapita)`
...
Operating theatre procedures done per year `r sum(toperating.theatre$byhfl$staff)` Operating theatre procedures per 100,000 population `r sum(oper.theatre.percapita)`
...
```{r}
allworkforces = sum(raw.workforce$`all workforce`)
```

All workforce: `r allworkforces` Workforce per 100,000 population: `r ytallwfpercapita`

All workforce over need `r gap.ytallwfpercapita`

The Lancet Commission on Global Surgery, indicated the global average number of operating theatres is approximately 20 workforces per 100,000 population. Cambodia currently are able to handle 25 workforce per 100,000 population annually— `r ytallwfpercapita` is over different from The Lancet Commission on Global Surgery the Lancet Commission on Global Surgery recommended benchmark—highlighting a significant increase in workforce for surgery.

### all workforce distributed by province

```{r, fig.width=9}
group_ggplot(ytallwf %>% 
               mutate(type = case_when(type == "cases per 100,000 population" ~ "Workforces per 100,000 population",
                                       type == "all workforce" ~ "Total Workforces", TRUE ~ type), 
                      geninfo_province = factor(geninfo_province, levels = rev(factor_province))),
             geninfo_province,staff,type, 
             "Number and rate per 100,000 population of Workforce performed per year", 
             "distributed by province",
             "",
             'none')
```
...
```{r}
raw_workforce = sum(raw.workforce$`SAO specialist`)
```

Specialist surgical workforce `r raw_workforce`

Specialist surgical workforce per 100,000 population `r saopercapita`

Specialist surgical workforce over need `r gap.saopercapita`
...
```{r}
sumsurgery = sum(raw.workforce$surgeons)
```

Surgeons workforce `r sumsurgery`

Surgeons workforce per 100,000 population `r surgeonspercapita`

Surgeons workforce unmeet need `r gap.surgeonspercapita`
...
```{r}
sumanesthesia = sum(raw.workforce$`anesthesia professionals`)
```

Anesthesia professionals workforce `r sumanesthesia`

Anesthesia professionals workforce per 100,000 population `r anesthesiapercapita`

Anesthesia professionals workforce unmeet need `r gap.anesthesiapercapita`
...
```{r}
sumallied = sum(raw.workforce$`allied health provider`)
```

Allied health provider workforce `r sumallied`

Allied health provider workforce per 100,000 population `r alliedhprovider.percapita`

Allied health provider workforce unmeet need `r gap.alliedhprovider.percapita`
...
Day-of-surgery mortality rate: NA
...
Total functioning Operating Rooms:

Functioning ORs per 100,000 population: `r ytOR.percapita`

Functioning ORs unmeet need: `r gap.ytOR.percapita`

The number of functioning ORs per 100,000 population is a key indicator of surgical capacity and access to essential healthcare services. However, this number varies greatly depending on a variety of factors related to a region's healthcare system and overall development. We identified 197 hospitals with surgical capacity with `r sum(OR$values, na.rm = TRUE)` total operating rooms `r ytOR.percapita` OR per 100,000 population
...
Total surgery bed: `r sum(bed.surgery$values, na.rm = TRUE)`

Surgery beds ratio per 100,000 population: `r per.surgerybed`
...
Total ICU beds: `r ICU.beds`

Total number of surgical beds: `r Surgical.beds`

Total beds in hospital: `r total.beds`

Percent of ICU beds by the total number of hospital beds: `r hosbed.icubed.ratio`

Percent of ICU beds by the number of surgical beds: `r surbed.icubed.ratio`
...
100% allways 5,000 procedures per 100,000 population 6.2 operating theatres per 100,000 A surgical bed ratio of approximately 0.03/1000 population
